...
% modifications made by Simon Henin, July, 2021
%   fix resample issue with non-multiple of 1000Hz
%   use fillmissing to remove nans
...
for cndlop = 1:length(eyedat)
    ...
    if size(eyedat{cndlop},2) > 500/samprate/1000
        ...
        if sum(isnan(x))==length(x),
            ...
            fixationstats{cndlop}.XY = [];
            fixationstats{cndlop}.variables = [];
            continue;
        end
        % remove preceeding/trailing nans and assign offset
        offset = 0;
        if isnan(x(1)),
            idx_ = find(diff(isnan(x))==-1, 1, 'first');
            offset = idx_;
            x(1:idx_) = [];
            y(1:idx_) = [];
        end
        if isnan(x(end)),
            idx_ = find(diff(isnan(x))==1, 1, 'last');
            x(idx_+1:end) = [];
            y(idx_+1:end) = [];
        end
        ...
        x = fillmissing(x, 'pchip'); % use shape-presewrving interpolatation
        ...
        x = resample(x,1000,1/samprate);%up sample to 1000 Hz
        y = resample(y,1000,1/samprate);%up sample to 1000 Hz
        ...
        try,
            ...
            fixationstats{cndlop}.fixationtimes = offset+fixationtimes;
            fixationstats{cndlop}.fixations = fixations;
            fixationstats{cndlop}.saccadetimes = offset+saccadetimes;
