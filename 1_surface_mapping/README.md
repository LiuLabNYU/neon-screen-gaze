...
Maps raw Neon gaze data onto screen pixel coordinates using AprilTag markers, with improved detection stability over the built-in Neon Player surface tracker.
...
- `neon_gaze_pipeline.py` — main surface mapping pipeline
- `surface_qa_video.py` — renders annotated QA video for visual inspection
...
Runs detection on all frames, caches results, and prints which tag ID appears at which screen corner. Check the output matches your setup. Detection cache is saved so this step does not need to be repeated.
...
| `--force-redetect` | False | Ignore cache and re-run detection |
...
- Use a tag size of at least 80×80 screen pixels, 100×100 recommended
- Keep a white quiet zone around each tag of at least the width of one tag module
- Keep the same tag IDs across all recordings so the surface definition is consistent

---

## Output CSV columns (gaze_on_surface.csv)
...
5. **Gaze mapping** — each gaze sample (in scene-camera pixel coordinates) is undistorted using the Neon scene camera calibration, then projected onto the screen using the inverse homography.
